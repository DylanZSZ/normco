...
def load_data(filename='../data/datasets/cl.obo', use_text_preprocesser=False):
    ...
    with open(file=filename, mode='r', encoding='utf-8') as f:
        ...
        return np.array(name_array), np.array(query_id_array), mention2id, edge_index,edges
...
#         filename = id+'.obo'
#
#         try:file = wget.download(url=url,out= os.path.join(data_dir,filename))
#         except:print("dataset downloading failure in id: {}, url: {} ".format(id,url))
#
...
#                 for K in range(start_index,end_index+1):#+1?
#                     mentions_test.append(mentions[K])
#                     try:mentions_train.remove(mentions[K])
#                     except: print(K)
#                     concepts_test.append(concept)
#                     try:concepts_train.remove(concept)
#                     except:print(K)
#
#
#                 left_concepts.remove(concept)
#             print(len(mentions_train),len(set(mentions_train)))
#             datasets_folds.append((mentions_train,concepts_train,mentions_test,concepts_test))
#
#             #check overlap
#             #for concept in concepts_test:
...
if __name__ == '__main__':
    #get_all_data()
    setup_seed(0)
    concept_list,concept2id,edges,mention_list,synonym_pairs = construct_graph('../data/datasets/apo.obo')
