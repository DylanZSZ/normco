...
class NormalizationModel(nn.Module):
    ...
    def __init__(self, num_diseases, disease_embeddings_init=None, phrase_embeddings_init=None,
                 distfn=scoring.EuclideanDistance(), rnn=nn.GRU, embedding_dim=200, output_dim=200, dropout_prob=0.0,
    ...
    def forward(self, inputs, coherence, joint=True):
        ...
        disease_embs = self.embeddings(inputs['disease_ids'].view(-1, nneg)).view(-1, dseq_len, nneg,
                                                                                  self.embedding_dim)
